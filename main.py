...
# The legacy schema owns its own `images`/`faces` tables, which clash with the SaaS platform schema.
# Point it at a separate database with LEGACY_DATABASE_URL when mounted via LEGACY_API_ENABLED=true.
db_url = os.getenv("LEGACY_DATABASE_URL") or os.getenv("DATABASE_URL") 
